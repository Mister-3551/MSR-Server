...
public interface HomeRepository extends JpaRepository<Home, Long> {
    ...
    @Query(value = "SELECT 0 AS id, u.username, s.rank, u.image, 0 AS total_users, COUNT(m.id) AS missions " +
            "FROM users u " +
    ...
    @Query(value = "SELECT 0 AS id, '' AS username, 0 AS rank, u.image, COUNT(u.id) AS total_users, (SELECT COUNT(id) FROM missions) AS missions " +
            "FROM users u " +
